...
function buildPixBlockHtml(info: BudgetPdfPaymentInfo): string {
  ...
  if (info.pixCopyPaste) {
    ...
    <div class="payment-block payment-block-pix">
      ...
      ${qrImg}
      <p class="payment-text payment-muted">Escaneie o QR Code no app do seu banco ou copie o código abaixo.</p>
      <p class="payment-pix-code">${escapeHtml(info.pixCopyPaste)}</p>
    </div>`;
  }
  ...
  return `
    <div class="payment-block payment-block-muted">
      ...
      <p class="payment-text payment-muted">Gerando PIX copia e cola… Salve o frete e aguarde alguns instantes.</p>
    </div>`;
}
...
<head>
  ...
  <style>
    ...
    .payment-pix-btn {
      display: inline-block; background-color: #059669; color: #ffffff;
      padding: 10px 16px; border-radius: 8px; font-weight: bold; font-size: 12px;
    }
    .payment-link-url a { color: #2563eb; word-break: break-all; font-size: 10px; }
    .payment-pix-code {
      font-family: monospace; font-size: 9px; word-break: break-all; background: #ffffff;
